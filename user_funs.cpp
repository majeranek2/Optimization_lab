...
matrix licz(double v0x, double omg)
{
	...
	while (abs(x - x_prev) >= 1e-6)
	{
		x += vx * dt;
		y = y0 - vy * dt;
		ax = (-0.5 * C * ro * S * vx * vx + ro * vy + omg * M_PI * pow(r, 3)) / m;
		ay = (-m * g - 0.5 * C * ro * S * vy * vy - ro * vx - omg * M_PI * pow(r, 3)) / m;
		vx += ax * dt;
		vy += ay * dt;
...
}matrix znajdz()
{
	...
	for (int v0x = -10; v0x < 11; v0x++) {
		for (int omega = -23; omega < 24; omega++) {
			xend = licz(v0x, omega);
			//xend = end;
			if (4 <= xend <= 6 && abs(det(xend) - 5) < abs(det(max_xend) - 5)) {
				max_xend = xend;
				best_v0x = v0x;
				best_omega = omega;
	...
	xend = max_xend, best_v0x, best_omega;
	return xend;
}
...
matrix ff3R(matrix x, matrix ud1, matrix ud2)
{
	...
	double best_v0x = 0;
	double best_omega = 0;
	x = max_xend, best_v0x, best_omega;
	x = znajdz();
	return x;
}
